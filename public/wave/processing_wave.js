...
void draw(){
  ...
  //println(timer);
    if(timer < 10000){      //shows the text for 5 secs after it starts
      textSize(32);
      background(40);
      fill(240);
...
void setup(){
  size(100,100);
  y = new int[width]; //which is 100//and they [0] [1] [2] ... are all empty rn
...
void draw(){
  ...
  //this overwrites data from right bc y[99]
  for(int i= 0; i< y.length-1; i++){
    y[i]= y[i+1];
  }
  y[99] = mouseY; 
  
  //this is data over writes from the left
  
  //for(int i= y.length-1; i>0; i--){    //as long as i is greater than 1, starting from 100
  //y[i] = y[i-1];                       // array [i] will be array [i-1] 
  //                                      //so 99 becomes 98, 98 becomes 87
  //}
  //y[0] = mouseY;             //first becomes mouseposition Y
  
  for(int i =1; i<y.length; i++){
    //point(i,y[i]);
    line(i,y[i],i-1,y[i-1]);  //or you can do this
  }
