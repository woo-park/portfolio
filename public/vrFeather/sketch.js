...
function preload() {
    bgSound = loadSound("sound/rainsound.mp3");
}
...
let containerText;
function textPanel(arg) {
	containerText = new Container3D({x:world.getUserPosition().x,y:world.getUserPosition().y,z:world.getUserPosition().z});

	world.add(containerText);
	...
	containerText.addChild(textHolder)

	// put another plane behind this one so we can see the plane's boundaries
	...
	containerText.addChild(backingPlane)
}
...
let r;
let g;
let containerPrimitives = new Container3D({x:0, y:0, z:-15});
function primitives() {
		world.add(containerPrimitives);
		function primitiveShapesDisplay() {
		 b = new Box({
			...
			containerPrimitives.addChild(b);
			...
			containerPrimitives.addChild(cl);
			...
			containerPrimitives.addChild(p);
		 ...
		 d = new Dodecahedron({
			...
			containerPrimitives.addChild(d);
			...
			containerPrimitives.addChild(o);
			 ...
			 c = new Circle({
				...
				containerPrimitives.addChild(c);
			...
			r = new Ring({
				...
				containerPrimitives.addChild(r);

		}
		primitiveShapesDisplay();
...
function containObjects() {
	...
	containerPrimitives.addChild(container);
...
function draw() {
	...
	// scaleObjects();
	containObjectsSpin();
	ranNudgeX = random(-0.2, 0.3)
	ranNudgeY = random(-0.2, 0.3)
	ranNudgeZ = random(-0.2, 0.3)
...
for (let q = 0; q < feathers.length; q++) {
	...
	// if (feathers[0].x) {
		...
		// let dis = dist(mycurPosition.x, mycurPosition.y, mycurPosition.z, feathers[q].myFeather.getX(), feathers[q].myFeather.getY(), feathers[q].myFeather.getZ())

		let dis = dist(mycurPosition.x, mycurPosition.y, mycurPosition.z, subContainer3d.getX(), subContainer3d.getY(), subContainer3d.getZ())

		// x:3, y:0.1, z:2		//where pillow located
		if (dis < 1) {
			// // alert('hit!!')
			// let temp = new Projectile();
			// projectiles.push( temp );
			if (int(frameRate()) % 13 == 0){
				inventory.bullet += 1;
				break;
			}
	...
	textHolder.tag.setAttribute('text',
	    `value: # of FEATHERS:\n${inventory.bullet}\n\n\nIF YOU WANNA SHOOT SOME FEATHERS\n\nSTAND ON TOP OF THE PILLOW\n\nLOAD UP\n\nCLICK\n\n(honestly just use arrow keys);width:8;color: rgb(0,0,0); align: center;`);


			// .spinY(0.1);
			let spinnedValue;
			spinnedValue = map(world.getUserRotation().y,-360,360,-3,3)
			constrain(spinnedValue,-3, 3)

			setTimeout(()=>{
				containerText.spinY(spinnedValue)
			},1000)

			if(containerText.rotationY >= world.getUserRotation().y) {

				spinnedValue = 0;
				containerText.rotationY = world.getUserRotation().y
			...
			// console.warn(world.getUserRotation().y, containerText.rotationY);

			// if(containerPrimitives){
				if(frameCount > 2000) {
					containerPrimitives.removeChild(cl);
					containerPrimitives.removeChild(p);
					containerPrimitives.removeChild(d);
					containerPrimitives.removeChild(o);
					containerPrimitives.removeChild(c);
					containerPrimitives.removeChild(b);
					containerPrimitives.removeChild(r);
					container.removeChild(b1);
					container.removeChild(b2);
				// }
			}
...
function projectilesMove() {
	...
	for (var j = 0; j < targets.length; j++) {
		...
		if (d < 2) {
			...
			hit = true;
			let ranSpinValueX = random(100, 360)
			let ranSpinValueY = random(100, 360)
			let ranSpinValueZ = random(100, 360)
			let randomNudge = random(0.02,0.2);



			if (int(random(0,2)) == 0) {		//0 or 1
				directionX = -1;
			} else {
				directionX = 1;
			}
			...
			// nudge(directionX * 0.3, 0.3, directionZ * 0.3)


			pillow.nudge(0,0.002,0)
			// targets[j].nudge(ranNudgeX,ranNudgeY,ranNudgeZ);
			targets[j].nudge(directionX * 0.3, 0.3, directionZ * 0.3)
